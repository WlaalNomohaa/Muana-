...
bot.on('interactionCreate', async (interaction) => {
  ...
  try {
    ...
    if (commandName === 'serverinfo') {
      ...
      const adminCount = members.filter(m => m.permissions.has(PermissionFlagsBits.Administrator) && !m.user.bot).size;
      const botAdminCount = members.filter(m => m.permissions.has(PermissionFlagsBits.Administrator) && m.user.bot).size;
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(`📌 *Kani waa macluumaadka guud ee Server-ka iyo xubnahiisa.*`)
        .setThumbnail(serverIcon)
        .addFields(
          { name: '👥 Total Members', value: `\`${guild.memberCount}\` xubnood`, inline: true },
          { name: '🎭 Total Roles', value: `\`${guild.roles.cache.size}\` roles`, inline: true },
          { name: '🛠️ Admins (Role Admin)', value: `👤 Users: \`${adminCount}\` | 🤖 Bots: \`${botAdminCount}\``, inline: false },
